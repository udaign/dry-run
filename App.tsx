...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const generateFaviconSvg = (color: string) => {
        const svgString = `<svg width="25" height="25" viewBox="0 0 25 25" xmlns="http://www.w3.org/2000/svg"><g fill="${color}">${Array.from({ length: 36 }, (_, i) => `<circle cx="${3.75 + (i % 6) * 3.5}" cy="${3.75 + Math.floor(i / 6) * 3.5}" r="1"/>`).join('')}</g></svg>`;
